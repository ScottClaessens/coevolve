...
The **coevolve** package supports using `nutpie` as an alternative high-performance
sampler. To use nutpie:
...
3. **Verify configuration**:
   ```r
   coevolve::check_nutpie_available()  # Should return TRUE
   ```

4. **Use nutpie** by setting `sampler = "nutpie"` in `coev_fit()`:
   ```r
   fit <- coev_fit(..., sampler = "nutpie")
   ```

**Note**: Explicit configuration is required to ensure reproducibility and avoid
version conflicts. The package does not automatically search for nutpie installations.
