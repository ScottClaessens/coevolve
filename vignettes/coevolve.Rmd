...
We can also plot some posterior predictive checks, to ensure that our model is
fitting to the data well.


``` r
#coev_plot_predictive_check(fit1)
```
...
previous examples, we had only one observation per taxa. But often there will
be more than one observation for each taxa, such as when we have observed
...
In the model output, we can see that `coev_fit()` detected the presence of
...
If we have data on the spatial or geographic location of taxa, sometimes it is
useful to control for this spatial location to ensure that our model is
capturing deep ancestral relationships rather than more recent diffusion among
neighbours. For example, when studying the coevolution of political and
religious authority in Austronesian societies, we would like to ensure that our
results are due to coevolution over deep cultural time rather than more recent
borrowing.
...
includes in the Stan code a Gaussian Process over spatial location for each
variable in the model.

Here is a geographic distance matrix (measured in metres) for the first five
...
```
Error in methods::is(object, "coevfit"): object 'fit4' not found
```
